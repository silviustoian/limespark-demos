...
(() => {
  ...
  function buildLogo() {
    ...
    // business.logoBg = background colour of the logo image (e.g. "#000" or "#fff").
    // When set, the logo sits on a chip of that colour so its edges blend on any
    // header/footer. Omit it for transparent PNGs (logo shows with no chip).
    const logoBg = get(CONTENT, 'business.logoBg');
    if (logoBg) document.documentElement.style.setProperty('--brand-logo-bg', logoBg);
    $$('[data-brand-logo]').forEach((img) => {
      const brand = img.closest('.brand');
      // if the logo file is missing/broken, fall back to the text mark
      img.onerror = () => {
        img.hidden = true;
        brand?.classList.remove('brand--logo', 'brand--chip');
        brand?.querySelector('.brand__mark')?.removeAttribute('hidden');
        brand?.querySelector('.brand__name')?.removeAttribute('hidden');
      };
      img.alt = alt;
      img.hidden = false;
      if (brand) {
        brand.classList.add('brand--logo');
        if (logoBg) brand.classList.add('brand--chip');
        // hide the fallback text mark while the logo is in place
        brand.querySelector('.brand__mark')?.setAttribute('hidden', '');
        brand.querySelector('.brand__name')?.setAttribute('hidden', '');
      }
      img.src = logo; // set last, so onerror is attached first
    });
  }
